...
public abstract class ConnectionPool implements Runnable {
   ...
   public Object getConnection() throws ConnectionException {
      return getConnection(100); // if 0 and waitifMaxout = true will cause a busy loop in recursive call
   }
   
   private static java.text.SimpleDateFormat df = new java.text.SimpleDateFormat("yyyyMMdd HH:mm:ss"); 
      ...
      if(connection == null) { // no available connections
         if(countConnections() < maxConnections) {
            ...
            if(waitIfMaxedOut) { 
               ...
               System.out.println("[" + df.format(new java.util.Date()) + "] awaken after wake timeout");
   ...
   public synchronized void kickout(Object conn)
   {
      inUseConnections.removeElement(conn);
