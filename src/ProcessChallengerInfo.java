...
public class ProcessChallengerInfo {
    public ProcessChallengerInfo() {
    }
    ...
    public static void main(String[] args) {
    	...
    	try {
	    	...
	    	process_login = new File(path, "WEB-INF/process_login");
	    	...
	    	/*
	    	 * 找目标表（tmp_hour4）最后一条。找到后以该小时为准汇入。
	    	 * 如果里面没资料，就不用管
	    	 * 如果里面有资料，要先把那一小时的资料删掉（因为要重新汇）
	    	 */
	    	Date d = null;
    		System.out.print("## a ");
	    	List<TmpHour3> tmps = tmgr.retrieveList("", "order by id desc limit 1");
	    	...
	    	if (tmps.size()>0) {
	    		...
	    		System.out.println("#found tmp_hour4 last record " + 
	    				sdf2.format(d) + " deleting from " + tmps.get(0).getId());
	    		tmgr.executeSQL("delete from tmp_hour4 where id>=" + tmps.get(0).getId());
	    		System.out.println("#done deleting");
	    	}
	    	if (d==null) {
	    		System.out.println("cannot find export point in tmp_hour4");
	    		System.exit(0);
	    	}
